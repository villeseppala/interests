...
I am creating a website that allows the user to input existential risk probabilities for different time periods. The site then calculates the expected longevity of humankind (measured in expected years the humankind will exist or in expected human lives left, for example). Results are accompanied by graphs.

The motivation behind the site is to help demonstrate in a concise and easily digestible manner that even very small improvements in existential risk will pay off immensely in the long run, in terms of humankind’s longevity. This should encourage us to pursue these improvements in various ways.
...
They layout below shows another type of layout that focuses more on graphical representation and different timescales. It also shows different options on the left hand side menu.
