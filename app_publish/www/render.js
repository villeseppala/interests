...
var nodeOutlineWidth = 3; // resting node border width in px; highlight widths scale from this (author-controllable)
...
var _zoomFocal = null;       // { cx, contentX } — keep a zoom's focal point under the cursor
    ...
    cyLib('core', 'nodeHtmlLabel', function (opts) {
      ...
      ['mousedown','mouseup','dblclick','touchstart','touchend'].forEach(function (evt) {
        ov.addEventListener(evt, function (e) {
          var t = e.target;
          if (t && t.closest && (t.closest('.inline-node-desc') || t.closest('.inline-copy-link') || t.closest('.inline-article-link') || t.closest('.node-title-edit'))) e.stopPropagation();
        }, false);
      });
      ...
      ov.addEventListener('dblclick', function (e) {
        ...
        if (descEl && !(t.closest && t.closest('.inline-article-link'))) {
          e.stopPropagation(); e.preventDefault();
...
function layoutInlineScroll() {
  ...
  applyInlinePositions();             // stack expansions + apply per-column scroll offsets
  cy.emit('render');
...
// Set the inline magnification, keeping the point under the cursor (clientX) fixed, then re-fit.
function setUiZoom(newUi, clientX) {
  if (!cy || !inlineMode || useMobileLayout()) return;
  newUi = Math.max(1, Math.min(3, newUi));
  if (Math.abs(newUi - uiZoom) < 1e-4) return;
  var ga = document.getElementById('graph-area');
  if (ga && clientX != null) {
    var cx = clientX - ga.getBoundingClientRect().left;
    _zoomFocal = { cx: cx, contentX: (cx - cy.pan().x) / cy.zoom() };
  }
  uiZoom = newUi;
  layoutInlineScroll();
}
...
function applyInlinePositions() {
  ...
  Object.keys(cols).forEach(function (g) {
    ...
    // Clamp scroll to what still overflows after the upward shift
...
function buildStyle() {
  ...
  return [
    ...
    { selector: 'node.nbr-hi', style: {
        'border-width': (mobileMode ? 12 : 8) * bm, 'outline-width': 0,
    }},
    ...
    { selector: 'node.hovered', style: {
        'border-width': (mobileMode ? 14 : 9) * bm, 'outline-width': 0,
    }},
...
function nodeBorderBandsHtml(data) {
  ...
  if (!n || n.empty()) return '';
  var grp = data.group;
  var bm = nodeOutlineWidth / 3;        // highlight widths scale from the resting outline width
  var bw = (n.hasClass('selected') || n.hasClass('hovered')) ? (mobileMode ? 14 : 9) * bm
         : n.hasClass('nbr-hi') ? (mobileMode ? 12 : 8) * bm
         : nodeOutlineWidth;
  ...
  if (grp === 'Theme' || grp === 'Skill') {
    ...
    return '<div style="' + z + 'top:0;left:0;right:0;bottom:0;box-sizing:border-box;border:' + bw + 'px solid ' + col + ';"></div>';
  }
  ...
  var out = '';
  // If a connected Theme/Skill is hovered, highlight ONLY that node's band on this project's outline
  // (thicken it); every other segment stays at the resting width. Otherwise use the class-based width.
  ...
  var hiW = (mobileMode ? 12 : 8) * bm;               // highlighted-band outline width
  var restW = nodeOutlineWidth;                        // resting width for the non-highlighted parts
  function segW(which, i) {                            // width for a given left/right band segment
    if (!hlSide) return bw;
    return (which === hlSide && i === hlIndex) ? hiW : restW;
  }
  ...
  function edgeHalf(vpos, hpos, side, idx, col) {
    ...
    out += '<div style="' + z + hpos + ':0;' + vpos + ':0;width:50%;height:' + segW(side, idx) + 'px;background:' + col + ';"></div>';
  }
  ...
  function side(arr, which) {
    ...
    for (var i = 0; i < arr.length; i++)
      out += '<div style="' + z + which + ':0;top:' + geom[i].top + '%;height:' + geom[i].height + '%;width:' + segW(which, i) + 'px;background:' + arr[i] + ';"></div>';
  }
...
function nodeBodyHtml(data, noGradient) {
  ...
  if (g === 'Skill') {
    var html = '<div style="width:100%;height:100%;box-sizing:border-box;position:relative;' +
      'display:flex;flex-direction:column;justify-content:center;padding:4px 7px 4px ' + Math.max(7, accGutter) + 'px;overflow:hidden;">' +
  ...
  if (g === 'Project') {
    ...
    // Title side padding = accordion gutter + one extra symbol width, so the title isn't crowded
    // against the chevron (autoFitProjectWidth sizes the node to match this).
    var projSymW = _accSpec ? Math.round(accordionIconSize * (_accSpec.mult || 1)) : 0;
    var projPad = Math.max(9, accGutter) + projSymW;
    var availW = nodeW - ptypeColW - 2 * projPad;
    ...
    return '<div style="width:100%;height:100%;box-sizing:border-box;position:relative;overflow:hidden;' +
      ...
      '<div style="flex:1;display:flex;align-items:center;justify-content:center;padding:4px ' + projPad + 'px;' +
...
// "Read full article →" link, shown when the feature is on and this node has an article.
// `d` is a description record (staticNodeDescs entry / showDescPanel msg) carrying hasArticle+articleUrl.
function articleLinkHtml(d) {
  if (!articlesEnabled || !d || !d.hasArticle || !d.articleUrl) return '';
  var label = (currentLang === 'fi') ? 'Lue koko artikkeli →' : 'Read full article →';
  return '<a class="inline-article-link article-link" href="' + d.articleUrl + '">' + label + '</a>';
}
...
function inlineExpandedNodeHtml(data, exEntry) {
  ...
  var desc = '<div class="inline-node-desc" data-node-id="' + data.id + '" style="pointer-events:auto;user-select:text;-webkit-user-select:text;cursor:text;' +
    ...
    'padding:6px 10px 8px;position:relative;z-index:7;text-align:left;word-wrap:break-word;overflow-wrap:break-word;">' +
...
function measureDescHeight(node, descHtml) {
  ...
  _inlineProbe.style.width = Math.max(w - 20, 20) + 'px';  // desc padding is 10px each side
  ...
  return _inlineProbe.offsetHeight + 12 + 4; // desc vertical padding (4+8) + buffer
}
...
function measureRawHeight(node, raw) {
  ...
  var w = node.data('w') || 200;
  _inlineProbe.style.width = Math.max(w - 20, 20) + 'px';
  ...
  return _inlineProbe.offsetHeight + 12 + 4;
}
...
function expandNodeInline(id, descHtml, raw, lang, articleLink, skipReflow) {
  if (!cy) return;
  ...
  inlineExpandedMap[String(id)] = { descHtml: descHtml, h: origH + descH, origH: origH,
    raw: raw || '', lang: lang || 'en', articleLink: articleLink || '', openedAt: ++_openSeq };
...
// Expand a node inline from a description record {nodeId, text, text_fi, group, hasArticle, articleUrl}.
function expandNodeFromDesc(d, skipReflow) {
  ...
  var artLink = articleLinkHtml(d);
  expandNodeInline(d.nodeId, mdToHtml(dText) + artLink, dText, dLang, artLink, skipReflow);
}
...
function autoFitProjectWidth(data) {
  ...
  var _asym = _aspec ? Math.round(accordionIconSize * (_aspec.mult || 1)) : 0;
  var projPad = Math.max(9, _agut) + _asym;
  var requiredSingle = Math.ceil((maxTextW + 2 * projPad + 12) / (1 - typeF));
...
Shiny.addCustomMessageHandler('showDescPanel', function (msg) {
  ...
  if (mobileMode) {
    ...
    mdBody.innerHTML = mdToHtml(dText) + articleLinkHtml(msg);
    ...
    body.innerHTML = mdToHtml(dText) + articleLinkHtml(msg);
    applySidebarFonts();
...
function bindInlineWheel() {
  ...
  ga.addEventListener('wheel', function (e) {
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      setUiZoom((uiZoom || 1) * Math.exp(-e.deltaY * 0.005), e.clientX);  // 0.005 = half the old sensitivity
      return;
    }
    ...
    if (g && inlineBase) {              // only scrollable once something is expanded
  ...
  ga.addEventListener('touchmove', function (e) {
    ...
    if (_pinchD > 0) setUiZoom((uiZoom || 1) * Math.pow(d / _pinchD, 0.5), midX);  // sqrt = half response
    _pinchD = d;
  }, { passive: false });
